...
(
    ...
    mkdir -p "$INCL_DIR"
    cp include/bn.h  "$INCL_DIR"
    cp include/zm.h  "$INCL_DIR"
    cp include/zm2.h "$INCL_DIR"
    ...
    mkdir -p "$INCL_DIR/../depends/ate-pairing/"
    ln -sf "$INCL_DIR" "$INCL_DIR/../depends/ate-pairing/include"

    LIB_DIR=/usr/local/lib
    cp lib/libzm.a "$LIB_DIR"
    # NOTE: Not sure why, but getting error at runtime that this cannot be loaded. Maybe it should be zm.so?
    #cp lib/zm.so "$LIB_DIR/libzm.so"
)
...
(
    ...
    mkdir -p "$INCL_DIR"
    cp -r libfqfft/* "$INCL_DIR/"
    rm "$INCL_DIR/CMakeLists.txt"
)
